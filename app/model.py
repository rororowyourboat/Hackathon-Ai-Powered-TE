...
Investor_list = [
    Agents(type='Private', InvestorBool=True, unlocked_at_listing=25.0, lock_term_after_listing=3, release_term=24,
           perc_total_token_supply=np.round(public_round_size / system_params['total_token_supply'][0], 3),
           tokens=0).__dict__,
    Agents(type='Public', InvestorBool=True, unlocked_at_listing=0.0, lock_term_after_listing=0, release_term=6,
           perc_total_token_supply=np.round(private_round_size / system_params['total_token_supply'][0], 3),
           tokens=0).__dict__,
...
def p_monthly_unlocks(params, substep, state_history, prev_state, **kwargs):
    ...
    for agent in agents:
        if prev_state['timestep'] == agent['lock_term_after_listing']:
            new_tokens.append(
                agent['unlocked_at_listing'] * params['total_token_supply'] * agent['perc_total_token_supply'] / 100)
        elif (prev_state['timestep'] > agent['lock_term_after_listing'] and (
                prev_state['timestep'] <= (agent['lock_term_after_listing'] + agent['release_term']))):
            # new_tokens.append(((agent['perc_total_token_supply'])*params['total_token_supply'])/agent['release_term'])
            new_tokens.append((agent['perc_total_token_supply'] * (1 - (agent['unlocked_at_listing'] / 100))) * params[
                'total_token_supply'] / agent['release_term'])
